\documentclass[title, toc, copyright, labelbox, index]{hayektexreview}
\title{Read Me}
\titlepic[.2]{pic}
\subtitle{A Subtitle}
\stitle{an optional title for headers}
\note{And a note}
...
\begin{document}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item To change to symmetric margins, with 1.25 inches on each side, using the \code{center} class option.
	...
	\begin{itemize}
		\item Using the \code{index} class option, an link-able index of definitions will appear at the end of the document. "Index of Definitions" will also appear in the table of contents.
	...
	\begin{itemize}
		\item \readmedef{code} will generate the monospace font and color you see throughout this guide.
		\item \readmedef{mtr} will allow one to place margin text in the smaller margin, normally reserved for labels.
	...
	\begin{itemize}
		...
		\item Similarly, the color of page links, hyperlinks, and definitions hypertargets is controlled using \code{$\backslash$accentcolor\{$X$\}}.
		\item The color of the code box, which has usage \readmedef{code}, is controlled using \code{$\backslash$codecolor\{$X$\}}.
	\end{itemize}
