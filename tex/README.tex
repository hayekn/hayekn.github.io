\documentclass[title, toc, copyright, index]{hayektex}
\title{Template Guide}
\titlepic[.2]{pic}
\subtitle{A Subtitle}
...
\begin{document}
	...
	\begin{center}
		...
		\code{copyright} & creates copyright notice (with links) at bottom of table of contents\\
	...
	\begin{itemize}
		\item There are two ways to make this package recognizable by your computer:
		\begin{enumerate}
				\item Place \code{hayektex.cls} in the same folder as your \code{.tex} project. You will be able to load the class on this project only.
				\item 
					For Windows users, create or locate the directory
					$$\code{Users/<You>/texmf/tex/latex/local}$$
					For Mac users, create or locate the directory
					$$\code{Users/<You>/Library/texmf/tex/latex/local}$$
					Place \code{hayektex.cls} in the \code{local} folder. \code{<You>} should be able to load the class from anywhere on your computer. Check out \href{https://tex.stackexchange.com/}{the Tex stack exchange} for help. 
		\end{enumerate}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item By setting \readmedef{subtitle}, a subtitle will appear immediately below the title.
			\item By setting \readmedef{author}, an author will appear at the bottom of the title page. 
			\item By setting \readmedef{note}, an italicized note will appear at the bottom of the title page and below the author, if defined.
			\item By setting \code{$\backslash$titlepic[{<scale>}]\{<relative path>\}}, a picture will appear in the center of the title page, between the title/subtitle and author/note. 
		\end{itemize}
	\end{itemize}
	...
	\begin{itemize}
		...
		\item To create a copyright notice, use the \code{copyright} class option. A notice will appear at the bottom of the table of contents, in the format \textcopyright \; <Current Year> \, <Author>.
		\begin{itemize}
			\item By setting \readmedef{date}, one can customize the <Current Year> displayed.\mt{For demonstration, this guide is set with \code{\scriptsize$\backslash$date\{2\$$\backslash$tilde\{0\}\$26\}}}
			\item By setting \readmedef{email}, \href{mailto:you@youremail.com}{\faEnvelope} will appear to the right of the notice, where the envelope is a link to your email.
			\item By setting \readmedef{website},  \href{www.google.com}{\faGlobeAmericas} will appear to the right of the notice (and the email, if it exists), where the globe is a link to a website.
		\end{itemize} 
	\end{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item By setting \readmedef{stitle}, a custom (e.g. shorter) title will appear on even page headers in the place of the title defined by \readmedef{title}.
		\end{itemize}
	\end{itemize}
	...
	\\A page link is created that targets the location of the proposition label. It is accessed using \code{$\backslash$proplink\{<label ID>\}}. For example, \code{$\backslash$proplink\{2.1\}} will generate \proplink{2.1}.
	...
	\end{itemize}
	\section{Labels}
	All labels (generated from definitions, propositions, theorems, or \readmedef{say}) may be placed into a small, gray background box using the \code{labelbox} class option.\say{\colorbox{gray!10}{example}}
	\part{Customization}
	\section{Extra Commands and Options}
	\begin{itemize}
		\item \readmedef{part} will display the part name with a roman numeral.  \readmedef{part*} will only display the name. This does not affect how the current part is displayed in headers. All lower levels of sectioning are unnumbered.
...
\begin{itemize}
	...
	\item To help populate the index: \define{starting} \define{now}, \define{each} \define{word} \define{in} \define{this} \define{sentence} \define{is} \define{a} \define{new} \define{definition}.\margintext{Recall that labels are vertically aligned with the line in which the definition is invoked, so they overlap here.}
\end{itemize}
\end{document}